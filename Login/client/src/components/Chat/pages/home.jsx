...
import { setShowNotifications } from '../redux/profileSlice';
...
function Home() {
  ...
  const { activeUser } = useSelector((state) => state.activeUser); // Ensure correct state access
  const [searchResults, setSearchResults] = useState([]);
  const [isLoading, setIsLoading] = useState(false);
  const [search, setSearch] = useState("");
  const showNotifications = useSelector((state) => state.profile.showNotifications); // Ensure correct state access
          ...
          <div className="md:flex md:flex-col min-w-[360px] h-[100vh] md:h-[98.6vh] bg-[#ffff] relative">
            <div className='h-[61px] px-4'>
              ...
              <div className='absolute top-4 right-5 flex items-center gap-x-3'>
                ...
                <button className='flex items-center gap-x-1 relative'>
